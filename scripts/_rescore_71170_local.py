"""Rescore local asset_71170.mp4 with current cues; OpenAI-verify top stills."""
...
def main() -> int:
    ...
    print(
        f"cues: neg_w / ratio from CueScorer defaults; openai={openai_verify_enabled()}",
        flush=True,
    )
    ...
    while True:
        ...
        if frame_best is not None:
            ...
            if best_pass is None or frame_best.score > best_pass.score:
                best_pass = frame_best

    cap.release()
    print(f"frame_hits={len(hits)}", flush=True)
    _log("C", "_rescore_71170_local.py:hits", "frame_hits", {"n": len(hits), "thr": thr})
    if not hits:
        print("WOULD_ENTER_REVIEW_POSITIVE=False (no CLIP hits)", flush=True)
        return 0

    segs = aggregate_segments_dicts(hits, "asset_71170")
    print(f"segments={len(segs)}", flush=True)

    rows = []
    with tempfile.TemporaryDirectory(prefix="71170_") as td:
        tmp = Path(td)
        # Persist crops so write_sheet_from_crops / best_crop_path match production.
        ...
        for i, seg in enumerate(segs, 1):
            ...
            if wrote and Path(wrote).is_file():
                im = cv2.imread(str(wrote))
                sheet_shape = list(im.shape[:2]) if im is not None else None
            row = {
                "video_id": "asset_71170",
                ...
                "source_url": "https://www.britishpathe.com/asset/71170/",
    ...
    _log(
        "C",
        "_rescore_71170_local.py:summary",
        "rescore_done",
        {
            "keeps": keeps,
            "drops": drops,
